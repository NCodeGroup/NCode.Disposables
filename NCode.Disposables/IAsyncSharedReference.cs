...
/// <summary>
/// Represents an <see cref="IAsyncDisposable"/> resource that uses reference
/// counting and only disposes the underlying resource when all the
/// references have been released (i.e. reference count is zero).
/// </summary>
/// <remarks>
/// The very first instance of <see cref="IAsyncSharedReference{T}"/> will be
...
public interface IAsyncSharedReference<T> : IAsyncDisposable
{
    /// <summary>
    ...
    /// <exception cref="ObjectDisposedException">The reference count has reached zero (0)
    /// and the underlying resource has been disposed already.</exception>
    IAsyncSharedReference<T> AddReference();

    /// <summary>
    /// Attempts to increment the reference count and outputs a disposable resource that
    /// can be used to decrement the newly incremented reference count.
    /// </summary>
    /// <param name="reference">Destination for the <see cref="IAsyncSharedReference{T}"/> instance
    ...
    bool TryAddReference([MaybeNullWhen(false)] out IAsyncSharedReference<T> reference);
}