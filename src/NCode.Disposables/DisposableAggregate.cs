...
namespace NCode.Disposables
{
  ...
  {
    private static readonly IDisposable Sentinal = new DisposableEmpty();
    ...
    public void Dispose()
    {
      var disposable = Interlocked.Exchange(ref _disposable, Sentinal);
      disposable?.Dispose();
    }

    private IDisposable ThrowIfDisposed(IDisposable disposable)
    {
      if (ReferenceEquals(disposable, Sentinal))
    ...
    public IDisposable Disposable
    {
      ...
      get
      {
        var current = Interlocked.CompareExchange(ref _disposable, null, null);
      ...
      set
      {
        ...
        while (true)
        {
          var current = ThrowIfDisposed(_disposable);
